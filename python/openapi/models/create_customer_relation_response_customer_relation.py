...
class CreateCustomerRelationResponseCustomerRelation(BaseModel):
    """
    ...
    @field_validator('vayu_child_customer_id')
    def vayu_child_customer_id_validate_regular_expression(cls, value):
        """Validates the regular expression"""
        if not re.match(r"^[0-9a-fA-F]{24}$", value):
            raise ValueError(r"must validate the regular expression /^[0-9a-fA-F]{24}$/")
        return value
