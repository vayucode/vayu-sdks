 ...
 * no description
 */
export class CreditProductsApiRequestFactory extends BaseAPIRequestFactory {
     ...
     */
    public async listCreditProducts(_options?: Configuration): Promise<RequestContext> {
        let _config = _options || this.configuration;

        // Path Params
        const localVarPath = '/credit-products';
...
export class CreditProductsApiResponseProcessor {

    /**
     * Unwraps the actual response sent by the server from the response context and deserializes the response content
     * to the expected objects
     *
     * @params response Response returned by the server for a request to listCreditProducts
     * @throws ApiException if the response code was not in [200, 299]
     */
     public async listCreditProductsWithHttpInfo(response: ResponseContext): Promise<HttpInfo<ListCreditProductsResponse >> {
        const contentType = ObjectSerializer.normalizeMediaType(response.headers["content-type"]);
        if (isCodeInRange("200", response.httpStatusCode)) {
            const body: ListCreditProductsResponse = ObjectSerializer.deserialize(
                ObjectSerializer.parse(await response.body.text(), contentType),
                "ListCreditProductsResponse", ""
            ) as ListCreditProductsResponse;
