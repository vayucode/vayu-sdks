...
export class CreditsApiRequestFactory extends BaseAPIRequestFactory {
     ...
     */
    public async creditTopUp(creditTopUpRequest: CreditTopUpRequest, _options?: Configuration): Promise<RequestContext> {
        let _config = _options || this.configuration;

        // verify required parameter 'creditTopUpRequest' is not null or undefined
        if (creditTopUpRequest === null || creditTopUpRequest === undefined) {
            throw new RequiredError("CreditsApi", "creditTopUp", "creditTopUpRequest");
        }


        // Path Params
        const localVarPath = '/credits/top-up';
        ...
        const serializedBody = ObjectSerializer.stringify(
            ObjectSerializer.serialize(creditTopUpRequest, "CreditTopUpRequest", ""),
            contentType
        );
...
export class CreditsApiResponseProcessor {
     ...
     *
     * @params response Response returned by the server for a request to creditTopUp
     * @throws ApiException if the response code was not in [200, 299]
     */
     public async creditTopUpWithHttpInfo(response: ResponseContext): Promise<HttpInfo<CreditTopUpResponse >> {
        const contentType = ObjectSerializer.normalizeMediaType(response.headers["content-type"]);
        if (isCodeInRange("200", response.httpStatusCode)) {
            const body: CreditTopUpResponse = ObjectSerializer.deserialize(
                ObjectSerializer.parse(await response.body.text(), contentType),
                "CreditTopUpResponse", ""
            ) as CreditTopUpResponse;
